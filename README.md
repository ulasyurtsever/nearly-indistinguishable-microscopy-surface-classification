...
confusion concentrates on the LDPE and uv-PE pair, which the models represent as nearly
identical. The robustness and attention analyses show no dependence on vignetting or
illumination.
...
│       ├── robustness_and_attention.py   # corruption robustness + Grad-CAM border share, three seeds
...
└── results/
    ...
    │   └── tum_modeller_karsilastirma_zscore_FINETUNED.csv # all-models summary, fine-tuning stage
    ├── robustness/
    │   ├── robustness_summary_allseeds.csv     # nine conditions x three seeds, mean and SD
    │   ├── attention_border_allseeds.csv       # Grad-CAM border share with bootstrap CI, per seed
    │   ├── run_info.json
    │   └── seed42/, seed7/, seed123/           # per-seed CSVs, including baseline_predictions.csv
    ├── specimen_bootstrap_summary_seed42.csv
    └── predictions_with_specimen_seed42.csv
```

In each seed folder, `<Model>` covers the eight architectures evaluated at the feature-extraction stage (LeNet-5, Custom_Modern_Net, ResNet50-Scratch, VGG16, VGG19, InceptionV3, EfficientNetB0, ResNet50) plus the two fine-tuned models (ResNet50_FINETUNED and EfficientNetB0_FINETUNED). Custom_Modern_Net in the file names is the model called CustomNet in the paper.
...
2. **Attention and robustness analyses (need model + data).**
   ```bash
   python src/analysis/robustness_and_attention.py   # nine test-time conditions and Grad-CAM border shares, three seeds
   python src/analysis/specimen_bootstrap.py
   python src/analysis/gradcam_figure7.py
   python src/analysis/tsne_analysis.py
   ```
   `robustness_and_attention.py` reads the test images through the same TensorFlow
   input pipeline as training and compares its baseline predictions with the
   predictions saved during training before applying the perturbations.
